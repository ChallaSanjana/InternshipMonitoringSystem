...
import { getInternshipDisplayStatus, getInternshipDisplayStatusBadgeClass, getInternshipDisplayStatusLabel } from '../utils/internshipStatus';
...
export default function StudentDashboard() {
    ...
    const [reportsFilterInternshipId, setReportsFilterInternshipId] = useState('');
    ...
    const fetchReports = async (internshipId) => {
        try {
            setLoading(true);
    ...
    return (<div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 py-6 flex justify-between items-center">
          ...
          <button onClick={handleLogout} className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg transition">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        {success && (<div className="fixed right-4 top-20 z-50 rounded-lg bg-emerald-600 px-4 py-3 text-sm font-semibold text-white shadow-lg">
            {success}
          </div>)}
        ...
        <div className="mb-6 border-b border-gray-200">
          <div className="flex gap-4">
            <button onClick={() => setTab('internships')} className={`px-6 py-3 font-semibold border-b-2 transition ${tab === 'internships'
            ? 'border-blue-600 text-blue-600'
            : 'border-transparent text-gray-600 hover:text-gray-800'}`}>
              My Internships
            </button>
            <button onClick={() => setTab('reports')} className={`px-6 py-3 font-semibold border-b-2 transition ${tab === 'reports'
            ? 'border-blue-600 text-blue-600'
            : 'border-transparent text-gray-600 hover:text-gray-800'}`}>
              Progress Reports
            </button>
        ...
        {tab === 'internships' && (<div>
            <div className="mb-6">
              <button onClick={() => setShowAddInternship(true)} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-semibold transition">
                <Plus className="w-5 h-5"/>
                Add Internship
              </button>
            </div>

            {showAddInternship && (<AddInternship onClose={() => setShowAddInternship(false)} onSuccess={() => {
                    fetchInternships();
            ...
            {!loading && internships.length === 0 && (<div className="text-center py-12">
            ...
            <div className="grid gap-6">
              {internships.map((internship) => {
                ...
                return (<div key={internship._id} className="bg-white rounded-lg shadow p-6">
        ...
        {/* Reports Tab */}
        {tab === 'reports' && (<div>
            {showAddReport && selectedInternship && (<AddReport internshipId={selectedInternship} internshipStartDate={selectedInternshipDetails?.startDate} internshipEndDate={selectedInternshipDetails?.endDate} onClose={() => {
            ...
            {internships.length > 0 && (<div className="mb-6">
                <label className="block text-sm font-medium text-gray-700 mb-2">Select Internship</label>
                <select value={reportsFilterInternshipId} onChange={(e) => setReportsFilterInternshipId(e.target.value)} className="w-full md:w-96 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                  {internships.map((internship) => (<option key={internship._id} value={internship._id}>
            ...
            {!loading && reports.length === 0 && (<div className="text-center py-12">
                <p className="text-gray-600 text-lg">No reports submitted yet</p>
              </div>)}

            <div className="grid gap-6">
              {reports.map((report) => (<div key={report._id} className="bg-white rounded-lg shadow p-6">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => handleDeleteReport(report._id)} className="inline-flex items-center gap-1 rounded-lg border border-rose-300 bg-white px-3 py-1.5 text-xs font-semibold text-rose-700 transition hover:border-rose-400 hover:bg-rose-50">
                  ...
                  {report.adminFeedback && (<div className="bg-blue-50 border border-blue-200 rounded p-4 mt-4">
