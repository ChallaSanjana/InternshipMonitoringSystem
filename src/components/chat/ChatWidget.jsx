...
export default function ChatWidget() {
    ...
    const fetchMessages = useCallback(async (partnerStudentId) => {
    ...
    return (<>
      <button type="button" onClick={() => setIsOpen((previous) => !previous)} className="relative inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-300 bg-white text-slate-700 transition hover:bg-slate-100 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700" aria-label="Open chat" title="Chat">
        <span className="sr-only">Open chat</span>
        <MessageCircle className="h-5 w-5"/>
      </button>

      {isOpen && (<section className="absolute right-0 top-12 z-40 flex h-[460px] w-[340px] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900">
          <header className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-800">
            ...
            <button type="button" onClick={() => setIsOpen(false)} className="rounded-md p-1 text-slate-500 transition hover:bg-slate-200 hover:text-slate-700 dark:hover:bg-slate-700" aria-label="Close chat">
              <X className="h-4 w-4"/>
            </button>
          </header>

          {isMentor && mentorStudents.length > 0 && (<div className="border-b border-slate-200 px-3 py-2 dark:border-slate-700">
              <label className="mb-1 block text-xs font-semibold text-slate-500 dark:text-slate-400" htmlFor="mentor-student-chat-select">
                Chat with student
              </label>
              <select id="mentor-student-chat-select" value={selectedStudentId} onChange={(event) => setSelectedStudentId(event.target.value)} className="w-full rounded-lg border border-slate-300 px-2 py-2 text-sm">
                {mentorStudents.map((student) => (<option key={student._id} value={student._id}>
                    {student.name}
                  </option>))}
              </select>
            </div>)}
          ...
          <div ref={listRef} className="flex-1 space-y-2 overflow-y-auto bg-slate-50 px-3 py-3 dark:bg-slate-900/60">
            {loading && (<p className="text-xs text-slate-500 dark:text-slate-400">Loading messages...</p>)}

            {!loading && messages.length === 0 && (<p className="text-xs text-slate-500 dark:text-slate-400">No messages yet. Start the conversation.</p>)}

            {!loading && messages.map((item) => {
                const mine = item.senderId === user?._id;
                const isDeleting = deletingMessageId === item._id;
                return (<div key={item._id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                  <article className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${mine
                    ...
                    <div className="mt-1 flex items-center justify-end gap-2">
                      {mine && (<button type="button" onClick={() => onDeleteMessage(item._id)} disabled={Boolean(deletingMessageId)} className="inline-flex items-center rounded p-0.5 text-blue-100 transition hover:bg-blue-500/50 disabled:cursor-not-allowed disabled:opacity-60" aria-label="Delete message" title="Delete message">
          ...
          <footer className="border-t border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-900">
            ...
            <div className="flex items-center gap-2">
              <input type="text" value={messageInput} onChange={(event) => setMessageInput(event.target.value)} onKeyDown={(event) => {
                if (event.key === 'Enter') {
                    event.preventDefault();
                    onSendMessage();
                }
            }} placeholder="Type a message" className="flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm" disabled={loading || !target || sending}/>
              <button type="button" onClick={onSendMessage} disabled={loading || !target || sending || !messageInput.trim()} className="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-blue-600 text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50" aria-label="Send message">
