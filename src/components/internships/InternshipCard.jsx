...
export default function InternshipCard({ internship, onAddReport, onUploadFile, onDelete }) {
    ...
    return (<article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => onDelete(internship._id)} className="inline-flex items-center gap-1 rounded-lg border border-rose-300 bg-rose-50 px-3 py-1.5 text-xs font-semibold text-rose-700 transition hover:border-rose-400 hover:bg-rose-100">
      ...
      <div className="mt-5 flex items-center gap-3">
        <button onClick={() => onAddReport(internship._id)} disabled={!isApproved} title={!isApproved ? reportDisabledReason : 'Add a progress report'} className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50">
          <FileText className="h-4 w-4"/>
          Add Report
        </button>
        <button onClick={() => onUploadFile(internship._id)} disabled={!canUploadFile} title={!canUploadFile ? uploadDisabledReason : 'Upload supporting file'} className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50">
          Upload File
        </button>
      </div>
