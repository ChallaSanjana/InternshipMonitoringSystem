...
export default function ReportList({ reports, onEdit, onDelete }) {
    if (reports.length === 0) {
        return (<div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500">
          ...
          <tbody className="divide-y divide-slate-100">
            {reports.map((report) => (<tr key={report._id}>
                ...
                <td className="px-5 py-4 text-sm">
                  {report.adminFeedback ? (<div className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-800">
                      {report.adminFeedback}
                    </div>) : (<span className="text-xs text-slate-500">No feedback yet</span>)}
                </td>
                <td className="px-5 py-4 text-sm">
                  {report.mentorFeedback ? (<div className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-800">
                    ...
                    </div>) : (<span className="text-xs text-slate-500">No mentor feedback yet</span>)}
                </td>
                <td className="px-5 py-4 text-sm">
                  <div className="flex gap-2">
                    <button onClick={() => onEdit(report)} className="inline-flex items-center gap-1 rounded-lg border border-blue-300 bg-white px-3 py-1.5 text-xs font-semibold text-blue-700 transition hover:border-blue-400 hover:bg-blue-50">
                      <Pencil className="h-3.5 w-3.5"/>
                      Edit
                    </button>
                    <button onClick={() => onDelete(report._id)} className="inline-flex items-center gap-1 rounded-lg border border-rose-300 bg-white px-3 py-1.5 text-xs font-semibold text-rose-700 transition hover:border-rose-400 hover:bg-rose-50">
