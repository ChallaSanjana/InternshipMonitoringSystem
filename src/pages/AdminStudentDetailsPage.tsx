    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start gap-4">
          {student.profileImage && !avatarError ? (
            <img
              src={getFilePreviewUrl(student.profileImage)}
              alt={student.name}
        ...
        {personalInfo.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {personalInfo.map((item) => (
              <div key={item.label} className={item.label === 'About' ? 'md:col-span-2' : ''}>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {internships.length === 0 ? (
          <p className="text-sm text-slate-600">No internships submitted.</p>
        ) : (
        ...
        {reports.length === 0 ? (
          ...
          <div className="space-y-4">
            {reports.map((report) => {
              ...
              return (
                <article key={report._id} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
