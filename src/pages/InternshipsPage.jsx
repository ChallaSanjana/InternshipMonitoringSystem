...
export default function InternshipsPage() {
    ...
    const [filesByInternship, setFilesByInternship] = useState({});
    ...
    const fetchFilesForInternship = async (internshipId) => {
        try {
    ...
    const handleDeleteFile = async (internshipId, fileId) => {
        try {
    ...
    return (<div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button onClick={() => setShowAddInternship(true)} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-700">
      ...
      {!loading && internships.length === 0 && (<div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500">
          No internships yet. Click Add Internship to get started.
        </div>)}

      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        {internships.map((internship) => (<div key={internship._id} className="space-y-3">
            <InternshipCard internship={internship} onAddReport={(internshipId) => {
            ...
            {(filesByInternship[internship._id] || []).length > 0 && (<div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
                <p className="mb-2 text-sm font-semibold text-slate-700">Uploaded Files</p>
                <FileList files={filesByInternship[internship._id] || []} loading={filesLoading[internship._id]} onDownload={handleDownload} onDelete={(fileId) => handleDeleteFile(internship._id, fileId)} onPreview={handlePreview}/>
              </div>)}
          </div>))}
      </div>
      ...
      {showAddReport && selectedInternshipId && (<AddReport internshipId={selectedInternshipId} internshipStartDate={selectedInternship?.startDate} internshipEndDate={selectedInternship?.endDate} onClose={() => {
      ...
      {showUploadFile && selectedInternshipId && (<UploadFileModal internshipId={selectedInternshipId} onClose={() => {
