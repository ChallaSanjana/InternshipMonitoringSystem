...
const getInternshipSortPriority = (internship) => {
    const now = Date.now();
...
const shouldShowProgress = (internship) => {
    if (internship.status !== 'approved') {
        return false;
    }
    return true;
...
export default function MentorDashboardPage() {
    ...
    const rows = useMemo(() => {
        return students.map((student) => ({
            ...
            internships: [...(student.internships || [])].sort((a, b) => {
    ...
    return (<div className="space-y-6">
          ...
          <table className="min-w-full">
            ...
            <tbody className="divide-y divide-slate-100">
              {rows.map((row) => (<tr key={row.id}>
                  ...
                  <td className="px-5 py-4 text-sm text-slate-700">
                    {row.internships.length === 0 ? (<span className="text-xs text-slate-500">No internship submitted</span>) : (<div className="space-y-3">
                        {row.internships.map((internship) => (<div key={internship._id} className="rounded-lg border border-slate-200 p-3">
                            ...
                            {shouldShowProgress(internship) && (<div className="mt-2 flex items-center gap-2">
                                <span className="text-xs font-semibold text-slate-600">Progress</span>
                                <div className="h-2 w-28 overflow-hidden rounded-full bg-slate-200">
                                  <div className="h-full bg-blue-600" style={{ width: `${Math.max(0, Math.min(100, internship.progress || 0))}%` }}/>
                                </div>
                  ...
                  <td className="px-5 py-4 text-sm text-slate-700">
                    <button type="button" onClick={() => handleViewDetails(row.studentId)} className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700 transition hover:bg-blue-100">
                      View Details
                    </button>
                  </td>
                </tr>))}
      ...
      {(detailsLoading || selectedStudentDetails || detailsError) && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
          <div className="max-h-[85vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button type="button" onClick={closeDetailsModal} className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-100">
                Close
              </button>
            </div>

            {detailsLoading && (<div className="flex min-h-40 items-center justify-center">
                <Loader2 className="h-7 w-7 animate-spin text-blue-600"/>
              </div>)}

            {!detailsLoading && detailsError && (<div className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-rose-700">
                {detailsError}
              </div>)}

            {!detailsLoading && !detailsError && selectedStudentDetails && (<div className="space-y-5">
                <section className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  {selectedStudentDetails.student.about && (<div className="mt-3 rounded-lg border border-slate-200 bg-white p-3">
                ...
                <section className="rounded-xl border border-slate-200 bg-white p-4">
                  ...
                  <div className="mt-3 space-y-3">
                    {selectedStudentDetails.internships.length === 0 && (<p className="text-sm text-slate-500">No internships submitted yet.</p>)}
                    {selectedStudentDetails.internships.map((internship) => (<div key={internship._id} className="rounded-lg border border-slate-200 p-3">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <p className="text-sm font-semibold text-slate-800">{internship.companyName}</p>
                          <span className={`rounded-full border px-2.5 py-1 text-xs font-bold ${getStatusClass(internship.status)}`}>
                            {getStatusLabel(internship.status)}
                ...
                <section className="rounded-xl border border-slate-200 bg-white p-4">
                  ...
                  <div className="mt-3 space-y-3">
                    {selectedStudentDetails.reports.length === 0 && (<p className="text-sm text-slate-500">No reports submitted yet.</p>)}
                    {selectedStudentDetails.reports.map((report) => (<div key={report._id} className="rounded-lg border border-slate-200 p-3">
