...
export default function MentorReportsPage() {
  const [reports, setReports] = useState<MentorReportItem[]>([]);
  ...
  const handleSaveFeedback = async (reportId: string) => {
                ...
                <tr key={report._id}>
                  ...
                  <td className="px-5 py-4 text-sm text-slate-700">
                    {report.mentorFeedback ? (
                      ...
                      <p className="mb-2 text-xs text-slate-500">No mentor feedback yet</p>
                    )}
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        disabled={feedbackSubmittingReportId === report._id}
                        className="rounded-lg border border-blue-300 bg-white px-3 py-1.5 text-xs font-semibold text-blue-700 transition hover:border-blue-400 hover:bg-blue-50 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                  ...
                  <td className="px-5 py-4 text-sm text-slate-700">
                    {report.reportFile?.fileUrl ? (
                      <a
                        href={getFilePreviewUrl(report.reportFile.fileUrl)}
                        target="_blank"
                      ...
                      <span className="text-xs text-slate-500">No report file</span>
                    )}
                  </td>
                  <td className="px-5 py-4 text-sm">
                    {report.mentorReviewed ? (
                  ...
                  <td className="px-5 py-4 text-sm">
                    <button
                      type="button"
                      onClick={() => handleMarkReportReviewed(report._id)}
                      disabled={report.mentorReviewed || reviewingReportId === report._id}
