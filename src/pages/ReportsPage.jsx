...
export default function ReportsPage() {
    ...
    const addReportDisabledMessage = selectedInternship?.status === 'pending'
    ...
    const fetchInternships = async () => {
        ...
        const hasQueryInternship = internshipIdFromQuery && items.some((item) => item._id === internshipIdFromQuery);
    ...
    return (<div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {selectedInternshipId && (<button onClick={() => {
                if (!canAddReport) {
                    return;
                }
                setShowAddReport(true);
            }} disabled={!canAddReport} title={!canAddReport ? addReportDisabledMessage : 'Add report'} className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50">
            <FileText className="h-4 w-4"/>
            Add Report
          </button>)}
      </div>

      {!canAddReport && selectedInternshipId && addReportDisabledMessage && (<p className="rounded-xl bg-amber-50 p-3 text-sm text-amber-700">{addReportDisabledMessage}</p>)}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <select value={selectedInternshipId} onChange={(e) => {
      ...
      {showAddReport && selectedInternshipId && canAddReport && (<AddReport internshipId={selectedInternshipId} internshipStartDate={selectedInternship?.startDate} internshipEndDate={selectedInternship?.endDate} onClose={() => setShowAddReport(false)} onSuccess={() => {
      ...
      {showEditReport && selectedReport && selectedInternship && (<EditReport report={selectedReport} internshipStartDate={selectedInternship.startDate} internshipEndDate={selectedInternship.endDate} onClose={() => {
